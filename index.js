...
function addTenToEach(array) {
  ...
  // Виводимо результат
  let list = Array(array.lenght);
  for (let element in array) {
    list[element] = array[element] + 10;
  }
  console.log(list);
}
...
function multiplyByIndex(arr) {
  ...
  for (let element in arr) {
    list[element] = arr[element] * element;
  }
  return list;
}
...
function replaceNumbers(arr) {
  ...
  for (let element in arr) {
    if (arr[element] > 10) {
      list[element] = `Greater than 10`;
    } else {
      list[element] = `Less than or equal to 10`;
    }
  }
  return list;
}
